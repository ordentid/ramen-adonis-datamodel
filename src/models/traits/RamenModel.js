...
class RamenModel {
  register(Model, customOptions = {}) {
    ...
    Model.updateObject = async function (id, data) {
      ...
      if (!genericModel) {
        throw new GenericResponseException(
          "DATA IS NOT EXIST FOR RELATED ID",
          null,
          404
        );
      }
    ...
    Model.saveObject = async function (data, genericModel) {
      ...
      if (columns.length == 0)
        throw new GenericResponseException(
          "MODEL ERROR. COLUMN IS NOT DEFINED IN RELATED MODEL",
          null,
          500
        );
      ...
      try {
        ...
        if (genericModel.id) {
          ...
          throw new GenericResponseException(
            "TRANSACTION ERROR OCCURED. ROLLBACK OPERATION " + error.message,
            null,
            500
          );
        }
        ...
        if (error.code == 23505) {
          throw new GenericResponseException(
            "PAYLOAD ERROR. PRIMARY KEY ALREADY EXISTS",
            null,
            422
          );
        }
    ...
    Model.saveRelations = async function (data, genericModel) {
      ...
      for (let i = 0; i < relations.length; i++) {
        ...
        switch (relation.type) {
          case "belongsToMany":
            await QueryResolver.saveBelongsToManyRelations(
              genericModel,
              relation.name,
              relationData
            );
            break;
          case "hasMany":
            await QueryResolver.saveHasManyRelations(
              genericModel,
              relation.name,
              relationData
            );
            break;
          default:
            await QueryResolver.saveHasOneRelations(
              genericModel,
              relation.name,
              relationData
            );
    ...
    Model.commonQueryBuilder = async function (builder, queryParams) {
      let defaultMeta = {message: "data successfully retrieved"};

      try {
        const queryResult = await QueryResolver.commonQueryBuilder(
          builder,
          queryParams
        );
    ...
    Model.assignSlug = function (obj, slugPath, slugValuePath) {
      ...
      newObj[slugPaths[lastIndex]] = value;
      return newObj;
    };
  }
}
